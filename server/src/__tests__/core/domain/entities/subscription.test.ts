...
describe("Subscription Entity", () => {
  ...
  describe("Constructor", () => {
    it("should create subscription with TokenTarget and PriceChangeStrategy", () => {
      ...
      const subscription = new Subscription("user-123", target, strategy);
    ...
    it("should create subscription with default active state when not specified", () => {
      ...
      const subscription = new Subscription("user-123", target, strategy); // No isActive parameter
    ...
    it("should create subscription with explicit active state", () => {
      ...
      const inactiveSubscription = new Subscription(
        "user-123",
        target,
        strategy,
        false,
  ...
  describe("shouldNotify Method - Price Change Strategy", () => {
    ...
    beforeEach(() => {
      ...
      subscription = new Subscription("user-123", target, strategy); // Uses default active=true
    });
    ...
    it("should work with absolute mode", () => {
      ...
      const sub = new Subscription("user-123", target, strategy); // Uses default active=true
  ...
  describe("notifyAndUpdate Method", () => {
    it("should create new subscription with updated target state", () => {
      ...
      const subscription = new Subscription("user-123", target, strategy); // Uses default active=true
    ...
    it("should work with different target types", () => {
      const nftTarget = new NftTarget(nftMeta, {
        lastNotifiedAt: new Date("2025-01-01"),
        lastNotifiedPrice: 5.0,
      });
      const strategy = new PriceChangeStrategy({
  ...
  describe("Immutability", () => {
    it("should maintain immutability throughout operations", () => {
      const originalTarget = new TokenTarget(tokenMeta, {
        lastNotifiedAt: new Date("2025-01-01"),
        lastNotifiedPrice: 100,
      });
      const strategy = new PriceChangeStrategy({
  ...
  describe("Activation and Deactivation", () => {
    ...
    beforeEach(() => {
      ...
      subscription = new Subscription("user-123", target, strategy, false);
    });
    ...
    describe("activate", () => {
      it("should activate an inactive subscription", () => {
        ...
        expect(subscription.isActive).toBe(false); // Original unchanged
      });

      it("should work on already active subscription", () => {
        const activatedSub1 = subscription.activate();
        expect(activatedSub1.isActive).toBe(true);

        const activatedSub2 = activatedSub1.activate();

        expect(activatedSub2.isActive).toBe(true);
      });
    });
    ...
    describe("activation state effects", () => {
      it("should allow shouldNotify when active", () => {
        ...
        expect(() => activated.shouldNotify(120)).not.toThrow();
        expect(activated.shouldNotify(120)).toBe(true);
      });

      it("should throw SubscriptionException when calling shouldNotify on inactive subscription", () => {
        expect(subscription.isActive).toBe(false);

        expect(() => subscription.shouldNotify(120)).toThrow(
          SubscriptionException,
        );
        expect(() => subscription.shouldNotify(120)).toThrow(
          "Domain logic error. Reason: subscription is not active",
        );
      });
      ...
      it("should allow notifyAndUpdate when active", () => {
        ...
        expect(() => activated.notifyAndUpdate(120)).not.toThrow();
        const updated = activated.notifyAndUpdate(120);
      ...
      it("should throw SubscriptionException when calling notifyAndUpdate on inactive subscription", () => {
        ...
        expect(() => subscription.notifyAndUpdate(120)).toThrow(
          SubscriptionException,
        );
        expect(() => subscription.notifyAndUpdate(120)).toThrow(
          "Domain logic error. Reason: subscription is not active",
      ...
      it("should preserve inactive state after updateStrategy", () => {
        ...
        expect(() => updated.shouldNotify(new Date())).toThrow(
          SubscriptionException,
        );
      });
    });
  ...
  describe("updateStrategy Method", () => {
    ...
    it("should work with inactive subscriptions", () => {
      ...
      const updated = deactivated.updateStrategy(newStrategy);
  ...
  describe("SubscriptionException Tests", () => {
    ...
    it("should throw SubscriptionException with correct message for shouldNotify", () => {
      try {
        ...
        expect((error as SubscriptionException).message).toBe(
          "Domain logic error. Reason: subscription is not active",
        );
      }
    });
    ...
    it("should not throw SubscriptionException for updateStrategy on inactive subscription", () => {
      ...
      expect(() =>
        inactiveSubscription.updateStrategy(newStrategy),
      ).not.toThrow();
  ...
  describe("Default Active State Behavior", () => {
    ...
    it("should demonstrate factory creates active subscriptions by default", () => {
      ...
      const explicitActive = new Subscription(
        "user-123",
        target,
        strategy,
        true,
      );
      const defaultActive = new Subscription("user-123", target, strategy);
  ...
  describe("Integration Tests with New Functionality", () => {
    it("should work in complete lifecycle with activation/deactivation", () => {
      ...
      const updated1 = activated.notifyAndUpdate(120);
      expect(updated1.isActive).toBe(true);
      expect(updated1.shouldNotify(140)).toBe(true);

      // Deactivate and test
      const deactivated = updated1.deactivate();
      expect(() => deactivated.shouldNotify(150)).toThrow(
        SubscriptionException,
      );

      // Update strategy while inactive
      const newStrategy = new TimeIntervalStrategy({ intervalMs: 3600000 });
      const updated2 = deactivated.updateStrategy(newStrategy);
      expect(updated2.isActive).toBe(false);
      expect(() => updated2.shouldNotify(new Date())).toThrow(
        SubscriptionException,
      );

      // Activate with new strategy
      const activated2 = updated2.activate();
      expect(() => activated2.shouldNotify(new Date())).not.toThrow();
    });
