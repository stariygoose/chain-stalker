...
import {
  IServerConfig,
  ServerConfig,
} from "#presentation/config/server.config.js";
import { ILogger, Logger } from "#utils/logger.js";
import {
  IMongoDbConfig,
  MongoDbConfig,
} from "#infrastructure/database/mongodb/config/mongo.config.js";
...
import {
  ...
  RedisClient,
  RedisPubSub,
} from "#infrastructure/lib/redis/index.js";
...
import {
  IStrategyService,
  StrategyService,
} from "#application/services/strategy.service.js";
...
import {
  CollectionService,
  ICollectionService,
} from "#application/services/collection.service.js";
...
container
  .bind<StrategyController>(TYPES.StrategyController)
  .to(StrategyController)
  .inSingletonScope();
container
  .bind<IStrategyService>(TYPES.StrategyService)
  .to(StrategyService)
  .inSingletonScope();
...
container
  .bind<TokenController>(TYPES.TokenController)
  .to(TokenController)
  .inSingletonScope();
container
...
container
  .bind<IJwtTokenRepository>(TYPES.JwtTokenRepository)
...
container
  .bind<IAuthenticator>(TYPES.Authenticator)
  .to(Authenticator)
  .inSingletonScope();
container
...
container
  ...
  .inSingletonScope();
container
  .bind<IMongoDbConfig>(TYPES.MongoDbConfig)
  .to(MongoDbConfig)
  .inSingletonScope();

container
  .bind<ITokenEventStream>(TYPES.BinanceEventStream)
  .to(BinanceEventStream)
  .inTransientScope();
container
  .bind<INftEventStream>(TYPES.OpenseaEventStream)
  .to(OpenseaEventStream)
  .inTransientScope();
container
